...
def init_openai_chat(temperature):
    return ChatOpenAI(model=os.getenv("CHAT_MODEL"), streaming=True, temperature=temperature)
...
def init_azure_chat(temperature):
    return AzureChatOpenAI(model=os.getenv("CHAT_DEPLOYMENT"), streaming=True, temperature=temperature)


def init_bedrock(temperature):
    return ChatBedrock(model_id=os.getenv("CHAT_MODEL"), streaming=True, model_kwargs={"temperature": temperature})
...
def init_cohere_chat(temperature):
    ...
    return ChatCohere(cohere_api_key=COHERE_API_KEY, model=COHERE_MODEL, temperature=temperature)
